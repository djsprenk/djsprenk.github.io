...
2. Ask the user whether this is a Major, Minor, or Patch bump using AskUserQuestion.
...
6. Update `CHANGELOG.md`:
   - Replace the `## [Unreleased]` heading with `## [Unreleased]` (empty, for
     ...
     `## [X.Y.Z] - YYYY-MM-DD` using today's date from the `currentDate` context.
   - Preserve all content that was under `[Unreleased]` — move it under the new
     version heading.
   - If `## [Unreleased]` has no content beneath it, still create the new version
